            ...
            {Object.entries(services).map(([key, service]) => (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-md font-medium transition-all duration-300 ${
                  activeTab === key ? "text-white shadow-md transform scale-105" : "hover:opacity-80"
                }`}
                style={{
          ...
          {Object.entries(services).map(([key, service]) => (
            <div
              ...
              className={`transition-all duration-500 ease-in-out ${
                activeTab === key ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full absolute inset-0"
              }`}
            >
